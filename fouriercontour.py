...
nn = 1000           # number of points defining a function when made here
...
for ttmp in range(tsRange):
    plt.plot(N.array(fig2[2][:ttmp+1]),
        N.array(fig2[1][:ttmp+1]),color=[0,1,0],linewidth=lw)

    for ii in range(1,2*mm+1):
        plt.plot(N.real(fig2[0][ttmp][ii-1:ii+1]),
            N.imag(fig2[0][ttmp][ii-1:ii+1]),color='magenta',linewidth=lw)
